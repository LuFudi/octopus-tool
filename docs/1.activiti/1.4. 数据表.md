...
DESCRIPTION_：任务描述（可以将document的值设置成UEL表达式，动态设置描述，例如待办/已办任务的自定义标题）_
TASK_DEF_KEY_：流程图中的节点ID

OWNER_：任务的拥有者

ASSIGNEE_：任务的办理人

DELEGATION_：任务委托状态。任务被委托时，为PENDING，委托任务被解决后为RESOLVED
...
CLAIM_TIME_：任务被拾取的时间

### 3.4 ACT_RU_JOB：运行时作业

### 3.1 ACT_RU_EVENT_SUBSCR	：事件监听信息

当流程定义中配置了事件监听器，监听某个特定的事件时，会在 ACT_RU_EVENT_SUBSCR 表中添加相应的记录
